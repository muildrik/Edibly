...
router.post("/new", (req, res, next) => console.log(2) && res.json(JSON.stringify(newInstance(req.body))))
router.delete("/delete-recipe/:id", (req, res, next) => console.log(3) && res.json(JSON.stringify(database.delete(req.params.id))))
router.route("/update/:id").get((req, res) => console.log(4) && res.json(JSON.stringify(database.update(req.params.id, data))))
